<template>
  <form class='v-form'>
    ...
    <div class="agreement" v-if="config.agreement">
      <div class="agreement-icon" :class="{'checked': isCheckService}" @click="isCheckService = !isCheckService">
        <img src="https://images.vrm.cn/3u/2021/03/09/gou.png" class="agreement-icon__img" v-show="isCheckService">
      </div>
  ...
  methods: {
    ...
    submit() {
      this.formValidate(valid => {
        if (valid) {
          if (this.config.agreement.check && !this.isCheckService) {
            Toast.info('请勾选服务协议')
          } else {
    ...
    _maxlength(code) {
      ...
      const _getKey = Object.keys(length).find((key) => code === key)
      return length[_getKey]
    },
    // 初始化各组件配置
    options(field) {
      const { type, name, code } = field
      const text = type === 'input' ? '请输入' : '请选择'
      let placeholder = `${text}${name}`
      return {
        ...field,
        placeholder,
        type: type === 'input' ? (code.includes('mobile') ? 'tel' : 'text') : type,
        maxlength: this._maxlength(code)
      }
    },
    getTestCode () {
      this.testPhone()
